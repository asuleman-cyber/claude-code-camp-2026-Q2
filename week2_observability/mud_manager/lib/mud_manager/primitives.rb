module MudManager
  ...
  module Primitives
    ...
    # ---------- Admin (immortal-only; requires wizard level in-game) ----------
    #
    # Unlike the rest of this module, these are not part of the mortal command
    # surface and are never exposed as MCP tools — the daemon speaks only for
    # the player character. They exist for operator scripts (e.g.
    # week2_observability/bin/reset) that log in as an admin character directly via
    # MudManager::Session to manage world/player state.

    def admin_goto(target)
      require_str!(target, :target)
      cmd(:admin_goto, "goto", "goto #{target}", target: target)
    end

    def admin_transfer(target)
      require_str!(target, :target)
      cmd(:admin_transfer, "trans", "trans #{target}", target: target)
    end
