...
- The room-graph store built in Week 2 kept a full map, but exactly one thing could read it: the state block for the room the agent was standing in. No role could ask about anywhere else.
- Everything learned died with the process. Restarting meant relearning that the pit fiend kills you.
- `Boukensha::Permissions` had been built and tested in Week 2, then left switched off — no task had ever needed a restricted tool surface.
...
- Is an LLM subagent ever right for navigation, when the room graph already supports an exact shortest-path query? Week 2's navigation fix had already deleted an LLM room-inspector in favour of a parser for that reason.
...
### 1. The plan had to live in the system prompt (orchestrator)
...
- `#system` aliased to the composed value, so all five backends picked it up with zero per-backend code — same trick `#messages` already used for the room-graph state block.

> Test fills a context, compacts it, asserts the plan is still in the prompt. That's the guarantee, not the comment.

### 2. Subagents share the Player's MUD connection (orchestrator)
...
### 3. Read-only meant Permissions, not a new mechanism (orchestrator — Judge)
...
- Adding one meant a second gate beside the existing Permissions allowlist. Used `Tasks::Judge::READ_ONLY_TOOLS` fed through `Permissions` instead — one gate, enforced in `Registry#tool`/`#dispatch` like everything else.
...
> The Permissions engine, built in Week 2 and unused since, finally has a real caller. Denied isn't refused — it's invisible.

### 4. A dead code path had rotted since Week 1 (found building the orchestrator)
...
### 5. Knowledge became queryable instead of only injected (knowledge query API)
...
### 6. The Navigator had to justify existing — then never got called (Navigator)
...
> The tool description steered correctly — that's the design working, not the Player ignoring it. But it leaves the Navigator's justification an argument, not a finding. Two sessions produced none of the cases it exists for.

### 7. Memory is a different question from world state (memory)
...
- `<name>.jsonl` append-only history + `<name>.md` rewritten wholesale. Open-append-close everywhere — a held handle blocks deletion on Windows, the same file-handle bug hit earlier by the logger and the room-graph store.
...
### 8. Live run — the Judge disagreed (orchestrator, knowledge API, and memory)
...
- Every hooked session since the room-graph store shipped in Week 2 rendered `[here] Poor Alley…` where the human's instruction belonged. Nothing failed — it *is* a real message the model really received.
