...
**Status: infra + instrumentation both live (Phases 1–2 done).** See
[`docs/otel.md`](../../docs/otel.md) for the run-it-yourself tutorial and
[`docs/plans/otel_integration_plan.md`](../../docs/plans/otel_integration_plan.md)
for the design. Phase 3 (a trace-id deep link in `mud_monitor`'s session
view) is still optional/pending.

Boukensha emits metadata-only traces and keeps its existing JSONL session
log — tracing is additive, never a replacement. The instrumentation
contract is pinned to **OpenTelemetry GenAI semantic conventions 1.37.0**.
Attribute changes require an explicit compatibility review even when SDK
...
```sh
docker compose -f week3_capable/observability/docker-compose.yml --profile debug up
docker compose -f week3_capable/observability/docker-compose.yml --profile jaeger up
docker compose -f week3_capable/observability/docker-compose.yml --profile tempo up
docker compose -f week3_capable/observability/docker-compose.yml --profile compare up
```

Stop the selected profile and remove its containers:

```sh
docker compose -f week3_capable/observability/docker-compose.yml --profile jaeger down
```
