...
class TestMcpServersConfig < Minitest::Test
  ...
  def test_returns_a_tool_count_per_server
    ...
    config_from(server_yaml("mud", extra: "    prefix: tbamud")) do |cfg|
      _ctx, registry = new_registry
      servers = Boukensha.send(:register_mcp_servers, registry, cfg)
      assert_equal({ "mud" => 27 }, Boukensha.send(:server_summary, servers))
    end
  ensure
    @fake&.stop
  end

  # Phase G: the live client rides along in the return value, because a
  # subagent has to reuse the Player's connection rather than spawn a second
  # one (a second `mud-manager --mcp` would be a second login as the same
  # character). Without this the Judge could only ever be toolless.
  def test_returns_the_live_client_and_prefix_per_server
    @fake = start_fake_mud
    config_from(server_yaml("mud", extra: "    prefix: tbamud")) do |cfg|
      _ctx, registry = new_registry
      servers = Boukensha.send(:register_mcp_servers, registry, cfg)
      ...
      assert_equal "mud",    servers.first[:name]
      assert_equal "tbamud", servers.first[:prefix]
      assert_respond_to servers.first[:client], :call_tool
    end
  ensure
    @fake&.stop
  end

  # The Judge's whole isolation guarantee rests on this: same connection,
  # separate Context, narrower tool surface.
  def test_subagent_context_reuses_the_connection_with_its_own_permissions
    @fake = start_fake_mud
    config_from(server_yaml("mud", extra: "    prefix: tbamud")) do |cfg|
      ctx, registry = new_registry
      servers = Boukensha.send(:register_mcp_servers, registry, cfg)

      judge_ctx, judge_registry = Boukensha.subagent_context(
        servers, permissions: Boukensha::Tasks::Judge.permissions,
        system: "judge", context_window: 100_000
      )

      # Its own Context — nothing it does can reach the player's history.
      refute_same ctx, judge_ctx
      assert_equal "judge", judge_ctx.system

      # Read-only surface: look survives, move and send_raw do not.
      names = judge_registry.tool_names
      assert_includes names, "tbamud__look"
      assert_includes names, "tbamud__consider"
      refute_includes names, "tbamud__move"
      refute_includes names, "tbamud__attack"
      refute_includes names, "tbamud__send_raw"

      # The player's own registry is untouched by the judge's narrowing.
      assert_includes registry.tool_names, "tbamud__move"
    end
  ensure
    @fake&.stop
  end
