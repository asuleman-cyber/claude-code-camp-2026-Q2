module Boukensha
  class Agent
    ...
    # How the last #run ended: :completed (the model stopped on its own),
    # :max_iterations, or :max_tokens. nil before the first run.
    #
    # The same three values already reach the session log via
    # Logger#turn_end(reason:); this exposes them to the *caller* too, which
    # is what lets an orchestrator (Boukensha::Session) tell "the agent
    # finished what it was doing" apart from "the agent was cut off
    # mid-thought" without parsing its own log back.
    attr_reader :stop_reason

    # task: the Tasks::Base subclass this agent is running as, recorded on
    # every response event so a session transcript can tell Planner, Player,
    # and Judge calls apart (Logger#response already had the field; until
    # Phase G there was only ever one task to put in it).
      ...
      @logger.in_span("boukensha.turn", attributes: { "boukensha.session_id" => @logger.session_id }) do
        ...
        loop do
          ...
          if parsed[:stop_reason] == "tool_use"
            ...
            @logger.response(text: text, usage: response["usage"], stop_reason: parsed[:stop_reason], task: @task, backend: @builder.backend)
    ...
    def wrap_up(reason)
      ...
      record_usage(response)
      @logger.response(text: text, usage: response["usage"], stop_reason: parsed_wrap[:stop_reason], task: @task, backend: @builder.backend)
    ...
    def handle_tool_calls(content, response)
      ...
      @logger.response(text: "(tool use — #{tool_calls.size} call#{'s' if tool_calls.size != 1})", usage: response["usage"], stop_reason: "tool_use", task: @task, backend: @builder.backend)
