module Boukensha
  module Tasks
    class Base
      ...
      # Is this task switched on in settings.yaml (`tasks.<name>.enabled`)?
      #
      # `default:` is what an absent key means. Planner and Judge pass false:
      # a settings.yaml written before Phase G must keep producing exactly the
      # single-agent behaviour it produced before, and turning on an extra
      # model call per turn is not something a config should do by surprise.
      # This is the same opt-in posture as Phase A's `allow:` engine.
      def self.enabled?(settings, default: false)
        value = fetch(settings, :enabled)
      ...
      class << self
        ...
        # Per-task default prompt (prompts/<task>/system.md) if the gem ships
        # one, else the shared prompt at the root (prompts/system.md).
        #
        # Until Phase G there was only one task, so one shared file was the
        # whole story; Planner and Judge each need their own prompt, and a
        # bare `prompts/system.md` can't be two things at once. The fallback
        # keeps `player` — which has no subdirectory — reading exactly the
        # file it always did.
