...
module MudManager
  module Mcp
    # A minimal stdio MCP client, kept for the daemon's OWN end-to-end tests
    # only (see docs/plans/mud_manager/generic_mcp_client.md §5d — the daemon
    # must not depend on boukensha, so this stays a small, deliberate
    # duplicate of Boukensha::Mcp::Client and should not grow features).
    class Client
      ...
      def handshake
        res = request("initialize", {
          ...
          "capabilities"    => {},
          "clientInfo"      => { "name" => "mud_manager-test-client", "version" => MudManager::VERSION }
        })
