...
module MudManager
  ...
  class Session
    ...
    def initialize(host: DEFAULT_HOST, port: DEFAULT_PORT, timeout: DEFAULT_TIMEOUT,
                   session_name: "default", telnet_log: TelnetLog.from_env)
      @host       = host
      @port       = port
      @timeout    = timeout
      @socket     = nil
    ...
    #
    # redact: true logs a placeholder instead of `line` in the telnet log
    # (used for the password line in #login) — the record still shows that
    # something was sent and its size, never what.
    def send_command(command, redact: false)
      ...
      @telnet_log&.chunk(session: @session_name, dir: "out", text: line, redacted: redact)
      @socket.write(line + "\r\n")
      line
    end
          ...
          loop do
            ...
            unless text.empty?
              @buffer_mu.synchronize do
                ...
                @buffer_cv.broadcast
                @telnet_log&.chunk(session: @session_name, dir: "in", text: text)
              end
            end
          end
